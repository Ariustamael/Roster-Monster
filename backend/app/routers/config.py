...
from ..schemas import (
    RankConfigCreate, RankConfigOut,
    CallTypeConfigCreate, CallTypeConfigOut,
)
...
def update_rank(rank_id: int, payload: RankConfigCreate, db: Session = Depends(get_db)):
    ...
    dup = db.query(RankConfig).filter(RankConfig.name == payload.name, RankConfig.id != rank_id).first()
    if dup:
        raise HTTPException(409, "Rank name already exists")
...
def list_call_types(db: Session = Depends(get_db)):
    ...
    for ct in rows:
        ...
        result.append(CallTypeConfigOut(
            id=ct.id,
            name=ct.name,
...
def create_call_type(payload: CallTypeConfigCreate, db: Session = Depends(get_db)):
    ...
    return CallTypeConfigOut(
        id=ct.id, name=ct.name, display_order=ct.display_order,
...
def update_call_type(ct_id: int, payload: CallTypeConfigCreate, db: Session = Depends(get_db)):
    ct = db.query(CallTypeConfig).get(ct_id)
    if not ct:
        raise HTTPException(404, "Call type not found")
    dup = db.query(CallTypeConfig).filter(CallTypeConfig.name == payload.name, CallTypeConfig.id != ct_id).first()
    if dup:
        raise HTTPException(409, "Call type name already exists")
    data = payload.model_dump(exclude={"eligible_rank_ids"})
    for k, v in data.items():
        setattr(ct, k, v)
    db.query(CallTypeEligibleRank).filter(CallTypeEligibleRank.call_type_id == ct_id).delete()
    for rid in payload.eligible_rank_ids:
